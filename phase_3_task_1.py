...
if data_document_count == 0:
    choice = str(input("No latent semantics found. Do you want to generate them now?(Y/N)"))
    if choice != 'N':
        print("Generating latent semantics")
        for i in range(101):
            print(i)
            rep_image = rep_image_collection.find_one({"target": i, "feature": feature})
            class_image_data = phase_2_features.find({"target": i})
            class_image_data = [image[feature] for image in class_image_data]

            distances = [math.dist(np.array(rep_image["feature_value"]).flatten(), np.array(feat).flatten()) for feat in class_image_data]
            top_ids = top_k_min_indices(distances, 10)

            features = [np.array(class_image_data[i]).flatten().tolist() for i in top_ids]

            collection.insert_one({"target": i, "rep_features": features, "feature": feature})
...
for image in query_image_features:
    ...
    for i in range(101):
        ...
        label_rep_scores = [float(math.dist(np.array(i).flatten(), np.array(image[feature]).flatten())) for i in image_data[0]["rep_features"]]
